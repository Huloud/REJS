float waterH(vec3 pos) {
	float noise = 0.0;
	vec2 wind = vec2(TIME) * 0.25 * WATER_SPEED;
	float yFactor = 0.5;
	noise+= texture(noisetex,(pos.xz + wind - pos.y * yFactor) / 1024.0 * 1.1).r * 1.0;
	noise+= texture(noisetex,(pos.xz - wind - pos.y * yFactor) / 1024.0 * 1.5).r * 0.8;
	noise-= texture(noisetex,(pos.xz + wind + pos.y * yFactor) / 1024.0 * 2.5).r * 0.6;
